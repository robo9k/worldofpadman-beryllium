...
static void SVC_Status( netadr_t from ) {
	...
	for (i=0 ; i < sv_maxclients->integer ; i++) {
		...
		if ( cl->state >= CS_CONNECTED ) {
			...
			team = (ps->pm_flags & PMF_FOLLOW) ? TEAM_SPECTATOR : ps->persistant[PERS_TEAM];
			Com_sprintf (player, sizeof(player), "%i %i %i %i \"%s\"\n", 
				ps->persistant[PERS_SCORE], cl->ping,
				team, ( (cl->netchan.remoteAddress.type == NA_BOT)? 1 : 0 ),
				cl->name );
...
void SVC_Info( netadr_t from ) {
	...
	// add "game" if a mod is running
	if( *gamedir && Q_stricmp(gamedir, BASEGAME) ) {
		Info_SetValueForKey( infostring, "game", gamedir );
	}

	// These are part of gamecode, but also needed by ui
