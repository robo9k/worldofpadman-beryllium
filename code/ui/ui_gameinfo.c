...
//		UI_LoadBotsFromFile("scripts/bots.txt");
	...
	numdirs = trap_FS_GetFileList("scripts", ".wopbot", dirlist, 1024 );
	dirptr  = dirlist;
	for (i = 0; i < numdirs; i++, dirptr += dirlen+1) {
		dirlen = strlen(dirptr);
		strcpy(filename, "scripts/");
...
void UI_LoadLogoForMenu(const char *logoname)
{
	if(uis.logo_loaded>63) return;

	Q_strncpyz(uis.logo_names[uis.logo_loaded],logoname,32);
	uis.logo_handles[uis.logo_loaded]=trap_R_RegisterShaderNoMip(va("spraylogos/%s",logoname));
	uis.logo_loaded++;
}

void UI_SearchLogos(void)
{
	char			dirlist[1024];
	char			tmpcvarstr[1024];
	char			*filestrptr;
	int				dirlen, numdirs, i;
	char			logoname[32];//reicht scho ;)

//jpg anfang
	Com_Printf("searching SprayLogos(jpg):\n");
	numdirs = trap_FS_GetFileList("spraylogos", ".jpg", dirlist, 1024 );
	filestrptr  = dirlist;
	tmpcvarstr[0]='\0';
	for (i = 0; i < numdirs; i++, filestrptr += dirlen+1) {
		dirlen = strlen(filestrptr);
		Q_strncpyz(logoname,filestrptr,32);
		*strstr(logoname,".jpg")='\0';
		Com_Printf("Logo:%.2i Name:%s\n",i,logoname);
		Q_strcat(tmpcvarstr,1024,va("%s\\",logoname));

		UI_LoadLogoForMenu(logoname);
	}
	Com_Printf("finished searching ... found %i logos(jpg)\n",numdirs);
//jpg ende
//tga anfang
	Com_Printf("searching SprayLogos(tga):\n");
	numdirs = trap_FS_GetFileList("spraylogos", ".tga", dirlist, 1024 );
	filestrptr  = dirlist;
//	tmpcvarstr[0]='\0';
	for (i = 0; i < numdirs; i++, filestrptr += dirlen+1) {
		dirlen = strlen(filestrptr);
		Q_strncpyz(logoname,filestrptr,32);
		*strstr(logoname,".tga")='\0';
		Com_Printf("Logo:%.2i Name:%s\n",i,logoname);
		Q_strcat(tmpcvarstr,1024,va("%s\\",logoname));

		UI_LoadLogoForMenu(logoname);
	}
	Com_Printf("finished searching ... found %i logos(tga)\n",numdirs);
//tga ende

	i=strlen(tmpcvarstr)-1;
	if(tmpcvarstr[i]=='\\')tmpcvarstr[i]='\0';

	trap_Cvar_Set("logolist",tmpcvarstr);
}
