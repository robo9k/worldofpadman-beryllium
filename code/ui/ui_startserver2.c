...
#define MAX_NAMELENGTH		32 //old: 16 ... hat nicht gereicht für die langen bb-mapnamen ;)
...
#define ID_SELECTBOTS			22

#define MAX_SELECTBOTS	20
...
static int gametype_remap[] = {GT_FFA, GT_TEAM, GT_TOURNAMENT, GT_SPRAYFFA, GT_SPRAY, GT_LPS, GT_BALLOON, GT_CTF };
//static int gametype_remap2[] = {0, 2, 0, 4, 1, 3, 5};
static int gametype_remap2[] = {0, 2, 0, 3, 5, 1, 7, 4/*, 6*/};//g_gametype -> gametype_items
...
static void StartServer_GametypeEvent( void* ptr, int event ) {
	...
	if(gametype_remap[s_startserver.gametype.curvalue]==GT_LPS)
		s_startserver.StartLives.generic.flags &= ~(QMF_INACTIVE|QMF_HIDDEN);
	else
		s_startserver.StartLives.generic.flags |= (QMF_INACTIVE|QMF_HIDDEN);
...
//			trap_Cvar_SetValue ("capturelimit", Com_Clamp( 0, flaglimit, flaglimit ) );
			...
			trap_Cvar_SetValue( "sv_punkbuster", s_startserver.punkbuster.curvalue );
...
//			trap_Cmd_ExecuteText( EXEC_APPEND, va( "wait ; wait ; map %s\n", s_startserver.maplist[s_startserver.currentmap] ) );
			...
			{
				if( gametype >= GT_TEAM ) {
					Com_sprintf( buf, sizeof(buf), "addbot %s %i %s\n", s_startserver.selectbotinfos[n].name, s_startserver.botskill,
						teamstrs[s_startserver.selectbotinfos[n].team] );
				}
				else {
...
//	UI_DrawString( x, y, s_startserver.maplist[n], UI_CENTER|UI_SMALLFONT, color_white );
	if(Menu_ItemAtCursor( b->generic.parent )==b)
		UI_DrawStringNS(x,y,s_startserver.maplist[n], UI_CENTER,14.0f,color_white);
	else
...
//	s_startserver.gametype.generic.name		= "Game Type:";
	...
	s_startserver.gametype.curvalue			= gametype_remap2[(int)Com_Clamp( 0, sizeof(gametype_remap2)/sizeof(int)-1, trap_Cvar_VariableValue( "g_gameType" ))];
...
//		s_startserver.mappics[i].focuspic			= GAMESERVER_SELECTED;
	...
	s_startserver.arrowleft.x					= 325-10-99;
	...
	s_startserver.arrowright.x					= 325+10;
...
#define OPTIONS_XPOS 502
	...
	s_startserver.fraglimit.generic.flags      = QMF_NUMBERSONLY|QMF_PULSEIFFOCUS|QMF_SMALLFONT;
	s_startserver.fraglimit.generic.x	       = OPTIONS_XPOS;
	s_startserver.fraglimit.generic.y	       = y;
//	s_startserver.fraglimit.generic.statusbar  = ServerOptions_StatusBar;
	...
	s_startserver.timelimit.generic.flags      = QMF_NUMBERSONLY|QMF_PULSEIFFOCUS|QMF_SMALLFONT;
	s_startserver.timelimit.generic.x	         = OPTIONS_XPOS;
	s_startserver.timelimit.generic.y	         = y;
//	s_startserver.timelimit.generic.statusbar  = ServerOptions_StatusBar;
...
//	{
		...
		s_startserver.friendlyfire.generic.flags= QMF_PULSEIFFOCUS|QMF_SMALLFONT|QMF_GRAYED;
...
//	}
	...
	s_startserver.pure.generic.flags			= QMF_PULSEIFFOCUS|QMF_SMALLFONT;
...
//	s_startserver.dedicated.generic.id			= ID_DEDICATED;
	s_startserver.dedicated.generic.flags		= QMF_PULSEIFFOCUS|QMF_SMALLFONT;
...
//	y += BIGCHAR_HEIGHT+2;
	s_startserver.punkbuster.generic.type			= MTYPE_SPINCONTROL;
	s_startserver.punkbuster.generic.name			= "Punkbuster:";
	s_startserver.punkbuster.generic.flags			= QMF_PULSEIFFOCUS|QMF_SMALLFONT;
	s_startserver.punkbuster.generic.id				= 0;
	s_startserver.punkbuster.generic.x				= OPTIONS_XPOS;
	s_startserver.punkbuster.generic.y				= y;
	s_startserver.punkbuster.itemnames				= punkbuster_items;
	s_startserver.punkbuster.curvalue = Com_Clamp( 0, 1, trap_Cvar_VariableValue( "sv_punkbuster" ) );

	y += BIGCHAR_HEIGHT+2;
	s_startserver.StartLives.generic.type       = MTYPE_FIELD;
	s_startserver.StartLives.generic.name       = "Start Lives:";
	s_startserver.StartLives.generic.flags      = QMF_NUMBERSONLY|QMF_PULSEIFFOCUS|QMF_SMALLFONT;
	if(gametype_remap[s_startserver.gametype.curvalue]==GT_LPS)
		s_startserver.StartLives.generic.flags |= QMF_INACTIVE|QMF_HIDDEN;
...
//	Menu_AddItem( &s_startserver.menu, &s_startserver.punkbuster );
...
#define MAX_SELECTLISTBOTS		8 //10
...
//	botSelectInfo.arrowright.generic.id			= ID_NEXTPAGE;
	...
	for(i=0;i<MAX_SELECTLISTBOTS;i++)
	{
		...
		botSelectInfo.selectedbotnames[i].style					= UI_SMALLFONT;
		...
		botSelectInfo.selectedbotteams[i].generic.flags			= QMF_SMALLFONT|QMF_CENTER_JUSTIFY;
		botSelectInfo.selectedbotteams[i].generic.x				= 580;
		...
		botSelectInfo.selectedbotteams[i].style					= UI_SMALLFONT|UI_CENTER;
	...
	botSelectInfo.BotSkill.generic.flags	= QMF_PULSEIFFOCUS|QMF_SMALLFONT;
	botSelectInfo.BotSkill.generic.name		= "Bot Skill:";
	botSelectInfo.BotSkill.generic.x		= 510;
	botSelectInfo.BotSkill.generic.y		= 400;
