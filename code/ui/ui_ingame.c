...
void InGame_Event( void *ptr, int notification ) {
	...
	case ID_RESTART:
		UI_ConfirmMenu( "RESTART MAP?", (voidfunc_f)NULL, InGame_RestartAction );
		break;
...
*/
	...
	if(UI_GetCvarInt("g_gametype")==GT_SPRAY || UI_GetCvarInt("g_gametype")==GT_SPRAYFFA)
	{
		y += INGAME_MENU_VERTICAL_SPACING;
		...
		s_ingame.rechooselogo.string				= "RECHOOSE LOGO";
		s_ingame.rechooselogo.color					= colorDkGreen;
		s_ingame.rechooselogo.focuscolor			= colorGreen;
		s_ingame.rechooselogo.style					= UI_CENTER|UI_SMALLFONT;
	}

	y += INGAME_MENU_VERTICAL_SPACING;
	s_ingame.music.generic.type				= MTYPE_TEXTS;
	s_ingame.music.fontHeight				= 20.0f;
	s_ingame.music.generic.flags			= QMF_CENTER_JUSTIFY;//|QMF_PULSEIFFOCUS;
	s_ingame.music.generic.x				= 320;
	s_ingame.music.generic.y				= y;
	s_ingame.music.generic.id				= ID_WOPMUSIC;
	s_ingame.music.generic.callback			= InGame_Event; 
	s_ingame.music.string					= "WoP MUSiC MENU";
	s_ingame.music.color					= colorBlack;
	s_ingame.music.focuscolor			= color_orange;
	s_ingame.music.style					= UI_CENTER|UI_SMALLFONT;
