...
static void PlayerSettings_SaveChanges( void ) {
	...
	trap_Cvar_Set("syc_logo",uis.logo_names[s_playersettings.slogo_num]);
	trap_Cmd_ExecuteText( EXEC_APPEND, va("selectlogo \"%s\"\n",uis.logo_names[s_playersettings.slogo_num]));
}
...
typedef struct
{
	...
	char		modelskinnames[MAX_SKINS][64];
	qhandle_t	skinicons[MAX_SKINS];
} ps_playericons_t;
...
/*
	...
	dirlist[38]='\0';
*/
	memcpy(dirlist,"padman\0padgirl\0monster\0padlilly\0fatpad",39);
	numdirs = 5;
	dirptr  = dirlist;
	tmpskinnum=0;
	for (i=0; i<numdirs && ps_playericons.nummodel < MAX_UIMODELS; i++,dirptr+=dirlen+1)
	{
		...
		for (j=0; j<numfiles && ps_playericons.lastskinicon[ps_playericons.nummodel] < MAX_SKINS;j++,fileptr+=filelen+1)
		{
			...
			{
				ps_playericons.skinicons[tmpskinnum]=trap_R_RegisterShaderNoMip(va("models/wop_players/%s/%s",dirptr,skinname));
				Com_sprintf(ps_playericons.modelskinnames[tmpskinnum],64,"%s/%s",dirptr,(char*)(&skinname[5]));
...
//special order ende
	...
	numdirs = trap_FS_GetFileList("models/wop_players", "/", dirlist, 2048 );
	dirptr  = dirlist;
//special order anfang
//(old)	tmpskinnum=0;
//special order ende
...
//special order anfang
		if(!Q_stricmp(dirptr,"padman") || !Q_stricmp(dirptr,"padgirl") || !Q_stricmp(dirptr,"monster") || !Q_stricmp(dirptr,"padlilly") || !Q_stricmp(dirptr,"fatpad"))
			continue;
//special order ende
...
static char modelname[32];
	...
	for(i=0;i<3;i++)
		s_playersettings.skin_icons[i].shader = ps_playericons.skinicons[i+s_playersettings.firstskin];
	...
	strncpy(modelname,UI_Cvar_VariableString("model"),32);

	for(i=0;i<32;i++) if(modelname[i]=='/') {modelname[i]='\0'; break;}
...
static void PlayerSettings_MenuEvent( void* ptr, int event ) {
	...
	case ID_MICON+4:
		...
		{
			if(strstr(ps_playericons.modelskinnames[i],"default")!=NULL)
			{
				char tmp[64],*chrptr;

				Q_strncpyz(tmp,ps_playericons.modelskinnames[i],64);
				if((chrptr=strchr(tmp,'/'))!=NULL) *chrptr='\0';
				trap_S_StartLocalSound(trap_S_RegisterSound(va("sounds/Playernames/%s.wav",tmp),qfalse),CHAN_LOCAL_SOUND);

				trap_Cvar_Set("model",ps_playericons.modelskinnames[i]);
				trap_Cvar_Set("headmodel",ps_playericons.modelskinnames[i]);
				break;
			}
		}
		break;
	...
	case ID_SICON+1:
	case ID_SICON+2:
		trap_Cvar_Set("model",ps_playericons.modelskinnames[tmpid-ID_SICON+s_playersettings.firstskin]);
		trap_Cvar_Set("headmodel",ps_playericons.modelskinnames[tmpid-ID_SICON+s_playersettings.firstskin]);
		break;
	case ID_PLAYERMODEL:
		if(trap_Key_IsDown(K_MOUSE1))
...
//		Com_Printf("Spraycolor=%i\n",(uis.cursorx-((menucommon_s*)ptr)->x)/14);
	...
	case ID_SLOGOPREV:
		...
		{
			s_playersettings.slogo_num--;
//			trap_Cvar_Set("syc_logo",uis.logo_names[s_playersettings.slogo_num]);
		}
		break;

	case ID_SLOGONEXT:
		if(s_playersettings.slogo_num<uis.logo_loaded-1)
		{
			s_playersettings.slogo_num++;
//			trap_Cvar_Set("syc_logo",uis.logo_names[s_playersettings.slogo_num]);
		}
...
//		s_playersettings.model_icons[i].generic.flags	= QMF_SILENT;
...

	{
		...
		trap_Cvar_VariableStringBuffer( "syc_logo",logoname,sizeof(logoname));

		if(logoname[0]=='\0')
			Q_strncpyz(logoname,"01_wop",sizeof(logoname));

		for(i=0;i<uis.logo_loaded;i++)
		{
			if(!Q_stricmp(uis.logo_names[i],logoname))
			{
