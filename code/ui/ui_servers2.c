...
#define ART_CREATE0				"menu/server/create0"
#define ART_CREATE1				"menu/server/create1"
...
#define ART_SERVERARROWUP0		"menu/server/arrowup0"
#define ART_SERVERARROWUP1		"menu/server/arrowup1"
#define ART_SERVERARROWDOWN0	"menu/server/arrowdown0"
#define ART_SERVERARROWDOWN1	"menu/server/arrowdown1"
#define ART_UNKNOWNMAP			"menu/server/unknownmap"
#define ART_REMOVE0				"menu/server/delete0"
#define ART_REMOVE1				"menu/server/delete1"
#define ART_PUNKBUSTER			"menu/art/pblogo" //TODO? remove pb-code
...
#define ID_ADDTOFAV			25
#define ID_ADDALLTOFAV		26
...
//#define GAMES_CTF			4
#define GAMES_SYCFFA		4
#define GAMES_SYCTP			5
#define GAMES_LPS			6
#define GAMES_BB			7
...
static const char *servertype_items[] = {
	...
	"Team Deathmatch",
	"Tournament",
//	"Capture the Lolly", // chang this -> have to change GAMES_* ! (some lines above this)
...
//#@: I don't thing any master-server realy use this XD ( tested with vq3-master )
...
static void ArenaServers_Insert( char* adrstr, char* info, int pingtime )
{
	...
	*/
	...
	else if( i > 8 ) { //noch zu ändern? magic number *uah*
		i = 9;
	}
	if( *s ) {
...
// I'll take this out for the standalone (#@ 070208)
//			if(strstr(Info_ValueForKey(info,"mapname"),"wop_")!=NULL || !Q_stricmp(Info_ValueForKey(info,"game"),"wop") || strstr(Info_ValueForKey(info,"game"),"padmod")!=NULL)
				// insert ping results
...
static void ArenaServers_StartRefresh( void )
{
	...
	*g_arenaservers.numservers       = 0;
	...
	if( g_servertype == AS_GLOBAL || g_servertype == AS_MPLAYER )
	{
// the wopMaster/httpMaster-code will be removed from the wop-engine ... so this doesn't make any sense any more
//		if(ui_useUDPserverlist.integer==0)
//		{
//			trap_Cmd_ExecuteText( EXEC_NOW, "serverlistWOP\n");
//		}
//		else
		{
...
void ArenaServers_SetType( int type )
{
	...
	case AS_LOCAL:
		...
		g_arenaservers.addTOfav.generic.flags &= ~(QMF_INACTIVE|QMF_HIDDEN);
		g_arenaservers.addAllTOfav.generic.flags &= ~(QMF_INACTIVE|QMF_HIDDEN);
...
// cpy from Favorites_Add (serverinfo)
void ArenaServers_AddToFavorites( const char* serverbuff )
{
	char	adrstr[128];
	int		i;
	int		best;
...
static void ArenaServers_Event( void* ptr, int event ) {
	...
	case ID_ADDTOFAV:
		...
		break;
	case ID_ADDALLTOFAV:
		for(value=0;value<g_arenaservers.list.numitems;value++)
			ArenaServers_AddToFavorites(g_arenaservers.table[value].servernode->adrstr);
		break;
	}
}
...
static sfxHandle_t ArenaServers_MenuKey( int key ) {
	...
	if (key == K_MWHEELDOWN) {
		int tmp = g_arenaservers.list.top+1;
		if(tmp<=(g_arenaservers.list.numitems-g_arenaservers.list.height))
			g_arenaservers.list.top=tmp;
	}
...
void Draw_ServerList( void *voidptr )
{
	int			x;
	int			u;
	int			y;
	...
	for( column = 0; column < l->columns; column++ ) {
		...
		for( i = base; i < base + l->height; i++) {
			...
			if (i == l->curvalue)
			{
				...
				UI_FillRect(u,y,l->width*SMALLCHAR_WIDTH-60,SMALLCHAR_HEIGHT+2,listbar_color);
...
//	trap_Cmd_ExecuteText( EXEC_APPEND, va( "globalservers 0 %d\n", (int)trap_Cvar_VariableValue( "protocol" ) ) );
	...
	g_arenaservers.list.generic.ownerdraw		= Draw_ServerList;
	...
	g_arenaservers.arrowup.generic.type		= MTYPE_BITMAP1024S;
	g_arenaservers.arrowup.x				= 900;
	g_arenaservers.arrowup.y				= 319;
	g_arenaservers.arrowup.w				= 38;
	g_arenaservers.arrowup.h				= 100;
	g_arenaservers.arrowup.shader			= trap_R_RegisterShaderNoMip(ART_SERVERARROWUP0);
	g_arenaservers.arrowup.mouseovershader	= trap_R_RegisterShaderNoMip(ART_SERVERARROWUP1);
	g_arenaservers.arrowup.generic.callback	= ArenaServers_Event;
	g_arenaservers.arrowup.generic.id		= ID_SCROLL_UP;

	g_arenaservers.arrowdown.generic.type	= MTYPE_BITMAP1024S;
	g_arenaservers.arrowdown.x				= 900;
	g_arenaservers.arrowdown.y				= 496;
	g_arenaservers.arrowdown.w				= 38;
	g_arenaservers.arrowdown.h				= 99;
	g_arenaservers.arrowdown.shader			= trap_R_RegisterShaderNoMip(ART_SERVERARROWDOWN0);
	g_arenaservers.arrowdown.mouseovershader= trap_R_RegisterShaderNoMip(ART_SERVERARROWDOWN1);
	g_arenaservers.arrowdown.generic.callback= ArenaServers_Event;
	...
	g_arenaservers.remove.x					= 800;//727;
	g_arenaservers.remove.y					= 110;
	...
	g_arenaservers.addTOfav.generic.type		= MTYPE_TEXTS;
	g_arenaservers.addTOfav.fontHeight			= 16.0f;
	g_arenaservers.addTOfav.generic.flags		= QMF_PULSEIFFOCUS;
	g_arenaservers.addTOfav.generic.callback	= ArenaServers_Event;
	g_arenaservers.addTOfav.generic.id			= ID_ADDTOFAV;
	g_arenaservers.addTOfav.generic.x			= 460;
	g_arenaservers.addTOfav.generic.y			= 60;
	g_arenaservers.addTOfav.string				= "add to Favorites";
	g_arenaservers.addTOfav.style				= UI_SMALLFONT;
	g_arenaservers.addTOfav.color				= color_orange;
	g_arenaservers.addTOfav.focuscolor			= color_red;

	g_arenaservers.addAllTOfav.generic.type		= MTYPE_TEXTS;
	g_arenaservers.addAllTOfav.fontHeight			= 16.0f;
	g_arenaservers.addAllTOfav.generic.flags		= QMF_PULSEIFFOCUS;
	g_arenaservers.addAllTOfav.generic.callback	= ArenaServers_Event;
	g_arenaservers.addAllTOfav.generic.id			= ID_ADDALLTOFAV;
	g_arenaservers.addAllTOfav.generic.x			= 460;
	g_arenaservers.addAllTOfav.generic.y			= 80;
	g_arenaservers.addAllTOfav.string				= "add ALL to Favorites";
	g_arenaservers.addAllTOfav.style				= UI_SMALLFONT;
	g_arenaservers.addAllTOfav.color				= color_orange;
	g_arenaservers.addAllTOfav.focuscolor			= color_red;
	...
	g_arenaservers.refresh.x				= 456;
	g_arenaservers.refresh.y				= 674;
	g_arenaservers.refresh.w				= 120;
	g_arenaservers.refresh.h				= 60;//75;
	...
	g_arenaservers.create.generic.type		= MTYPE_BITMAP1024S;
	g_arenaservers.create.x					= 605;
	g_arenaservers.create.y					= 674;
	g_arenaservers.create.w					= 120;
	g_arenaservers.create.h					= 60;//75;
	g_arenaservers.create.shader			= trap_R_RegisterShaderNoMip(ART_CREATE0);
	g_arenaservers.create.mouseovershader	= trap_R_RegisterShaderNoMip(ART_CREATE1);
	...
	g_arenaservers.punkbuster.generic.y				= 125;
	...
	g_arenaservers.pblogo.generic.y				= 155;
	...
	Menu_AddItem( &g_arenaservers.menu, (void*) &g_arenaservers.addTOfav );
	Menu_AddItem( &g_arenaservers.menu, (void*) &g_arenaservers.addAllTOfav );
...
void ArenaServers_Cache( void ) {
	...
	trap_R_RegisterShaderNoMip( ART_SERVERARROWUP0 );
	trap_R_RegisterShaderNoMip( ART_SERVERARROWUP1 );
	trap_R_RegisterShaderNoMip( ART_SERVERARROWDOWN0 );
	trap_R_RegisterShaderNoMip( ART_SERVERARROWDOWN1 );
