...
void CG_PredictPlayerState( void ) {
	...
	for ( cmdNum = current - CMD_BACKUP + 1 ; cmdNum <= current ; cmdNum++ ) {
		...
		if(cg.snap->serverTime==cg.time)
			ByteToDir(cg.snap->ps.stats[STAT_WWDIR],cg_pmove.wwnormal);//noch ändern (vielleicht kann man den int ganz nutzen)
		else
		{
			cg_pmove.wwnormal[0]=cg.wwnormal[0];
			cg_pmove.wwnormal[1]=cg.wwnormal[1];
			cg_pmove.wwnormal[2]=cg.wwnormal[2];
		}
		cg_pmove.lastwwframe=cg.lastwwframe;
#endif
...
#ifdef	WWCODE
//		cg.lastwwframe=cg_pmove.lastwwframe;//erst nach dem letzten cmd speichern
		cg.wwnormal[0]=cg_pmove.wwnormal[0];
		cg.wwnormal[1]=cg_pmove.wwnormal[1];
		cg.wwnormal[2]=cg_pmove.wwnormal[2];
#endif
...
#ifdef	WWCODE
	cg.lastwwframe=cg_pmove.lastwwframe;
#endif
