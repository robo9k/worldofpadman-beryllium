...
typedef struct flare_s
{
	...
	float		pos;
	vec4_t		color;
	//(//noch einbinden)draw only in special distances (0->start fade, 1->end fade and start fulldraw, 2->end fulldraw and start fade, 3->end fade)
...
void LF_Parser(const char* scriptname)
{
	...
	for(i=0;i<MAX_LFSCRIPTSIZE;i++)
	{
		...
		if(buffer[i]=='/')
		{
			if(buffer[i+1]=='/')
			{
				if(inString) { StrEndWork(); inString=qfalse; }
				for(;buffer[i]!='\n';i++);
				continue;
			}
			else if(buffer[i+1]=='*')
		...
		if(buffer[i]=='{')
		{
			...
			case PL_LF:
				...
				if(numlfs+1>=MAX_LENSFLARES) { Com_Printf("^1can't load lensflare-script: no struct free (MAX=%i)\n", MAX_LENSFLARES); return; }
			...
			case PL_FLARE:
				...
				if(!freeflares) { Com_Printf("^1can't finish load lensflare-script(\"%s\"): no flare-struct free (MAX=%i)\n",lfmem[numlfs-1].lfname, MAX_FLARES); return; }
			...
			if(!inString && buffer[i]=='\"')
			{
				...
				for(;buffer[i]!='\"';i++)
				{
					...
					if(tmpstrl<MAX_TMPSTRING-2)
					{
						...
						Com_Printf("^2tmpString is full (this may happen if you use very long names, >%i chars) ... if this leads to errors, short the names or/and send me a mail at raute_at@gmx.de\n",(MAX_TMPSTRING-2));
				}
...
// VV ... VV //get a info from the server which flares will be used(I don't know how I will make this exactly ... especially if I use ent-flares) ... I think I will inform me of the fixed lens and load the ent-lens depending on the registlist
// VV ... VV //loading all shaders needed for the lensflares
// I think I will load the shaders at the first use (sprites should not take that much loading time) ...
//*oh* and don't forget to save the scripts in vars (maybe only the needed ones) ??? öh? what ;)
//make a list of all lensflares in this map (name, origin/dir, dirbool) ... this list is only for fixed lensflares ... maybe we only need to do this vor skylfs
// ^^ INIT-stuff ^^
// vv ACTIVE-stuff vv
/*
#######################
...
void AddLFToDrawList(const char *lfname,vec3_t origin,vec3_t dir)
{
	...
	if(IFDA_firstempty==MAX_SCREENLFS) return;//noch ein meldung ausgeben?
...
A function for calculating the 2D-dir(center of the screen is 0/0) of a 3D-Dot/Dir ... base on my first lensflare src(for padmod)
returns alpha-float, if the lensflare can't be seen it is 0.0f
point is the 3d origin of the dot
dir is the direktion of the lf-source
refdef is the refdef for the drawn scene
v2 is the 2D-vector from the screen-center to the lf point
#######################
*/
static float Calculate_2DdirOf3D(vec3_t point, vec3_t dir, refdef_t *refdef, vec2_t v2,float *distanceSquared, vec4_t xywh)
{
	vec3_t	vec;
	vec3_t	axis[3];
	trace_t tr;
	...
	if(point[0]!=2300000.0f)//a small hack to mark only dirs (origins have to be between 65535 and -65535)
	{
		...
		if(tr.fraction!=1.0f) return 0.0f;
	...
	else
	{
		...
		if(!(tr.surfaceFlags&SURF_SKY)) return 0.0f;
	...
	if(vec[0]!=0.0f)//the normal formula doesn't work with vec[0]==0 ...
		v2[0]=(((vec[0]*axis[0][2]-vec[2]*axis[0][0])*(vec[1]*axis[2][0]-vec[0]*axis[2][1]))-((vec[0]*axis[0][1]-vec[1]*axis[0][0])*(vec[2]*axis[2][0]-vec[0]*axis[2][2])))/(((vec[0]*axis[1][1]-vec[1]*axis[1][0])*(vec[2]*axis[2][0]-vec[0]*axis[2][2]))-((vec[0]*axis[1][2]-vec[2]*axis[1][0])*(vec[1]*axis[2][0]-vec[0]*axis[2][1])));
	else//the formula is symmetric ... so I exchange [0] with [2] and [2] with [0]
		v2[0]=(((vec[2]*axis[0][0]-vec[0]*axis[0][2])*(vec[1]*axis[2][2]-vec[2]*axis[2][1]))-((vec[2]*axis[0][1]-vec[1]*axis[0][2])*(vec[0]*axis[2][2]-vec[2]*axis[2][0])))/(((vec[2]*axis[1][1]-vec[1]*axis[1][2])*(vec[0]*axis[2][2]-vec[2]*axis[2][0]))-((vec[2]*axis[1][0]-vec[0]*axis[1][2])*(vec[1]*axis[2][2]-vec[2]*axis[2][1])));
	if(vec[1]*axis[2][0]-vec[0]*axis[2][1])//don't diff with 0
		v2[1]=(vec[0]*axis[0][1]-vec[1]*axis[0][0]+v2[0]*(vec[0]*axis[1][1]-vec[1]*axis[1][0]))/(vec[1]*axis[2][0]-vec[0]*axis[2][1]);
	else//use a formular with an other "Definitionslücke"(definition-gap ?)
		v2[1]=(vec[0]*axis[0][2]-vec[2]*axis[0][0]+v2[0]*(vec[0]*axis[1][2]-vec[2]*axis[1][0]))/(vec[2]*axis[2][0]-vec[0]*axis[2][2]);
...
//	float	invprozSize=480.0f/xywh[3];
	...
	for(tmpflare=lfmem[lfid].firstflare;tmpflare!=NULL;tmpflare=tmpflare->next)
	{
		...
		if(!lfmem[lfid].shadersloaded)
		{
			...
			if(tmpflare->shader==0) tmpflare->shader=cgs.media.whiteShader;
		}
	...
	//				transparency=sqrt(tmpf)/tmpflare->specialVar;
		...
		else if(lfalpha<0.0f) continue;
		else tmpalpha2=lfalpha;

		tmpalpha=tmpflare->color[3];
		tmpflare->color[3]*=tmpalpha2;
		if(tmpflare->color[3]>1.0f) tmpflare->color[3]=1.0f;
...
*/
		...
		//				tmpf=asin(tmpdir[0])*180.0f/M_PI;//q3 has no asin-table =(
						...
						if(tmpdir[1]>0) tmpf=180.0f-tmpf;
					}
					else
						tmpf=0.0f;
				...
				tmpf+=tmpflare->turnstyle[0]+(tmpflare->turnstyle[2]*prozDir[0]*tmpflare->pos)+(tmpflare->turnstyle[3]*prozDir[1]*tmpflare->pos);

				AdvancedDrawPicA((xywh[0]+dir[0]*tmpflare->pos),(xywh[1]-dir[1]*tmpflare->pos),curRadius*2.0f,curRadius*2.0f,0,0,1,1,tmpflare->shader,tmpflare->color,tmpf,3);//TURNORIGIN_MIDDLECENTER=3
			}
			break;
		case SF_BEAM:
			{
				...
				if(tmpflare->pos>0 && curRadius>tmpflare->pos) curRadius=tmpflare->pos;
				else if(tmpflare->pos<0 && curRadius>-tmpflare->pos) curRadius=-tmpflare->pos;
				...
				Draw4VertsPic((xywh[0]+dir[0]*tmpflare->pos-qdir[0]+ldir[0]),(xywh[1]-(dir[1]*tmpflare->pos-qdir[1]+ldir[1])),
							(xywh[0]+dir[0]*tmpflare->pos-qdir[0]-ldir[0]),(xywh[1]-(dir[1]*tmpflare->pos-qdir[1]-ldir[1])),
							(xywh[0]+dir[0]*tmpflare->pos+qdir[0]-rdir[0]),(xywh[1]-(dir[1]*tmpflare->pos+qdir[1]-rdir[1])),
							(xywh[0]+dir[0]*tmpflare->pos+qdir[0]+rdir[0]),(xywh[1]-(dir[1]*tmpflare->pos+qdir[1]+rdir[1])),
							tmpflare->shader, tmpflare->color);
			}
			break;
		case SF_LINE:
			{
...
*/
				...
				tmpf+=(tmpflare->turnstyle[0]+(tmpflare->turnstyle[2]*prozDir[0]*tmpflare->pos)+(tmpflare->turnstyle[3]*prozDir[1]*tmpflare->pos))/180.0f*M_PI;
				...
				Draw4VertsPic((xywh[0]+dir[0]*tmpflare->pos-qdir[0]+ndir[0]),(xywh[1]-(dir[1]*tmpflare->pos-qdir[1]+ndir[1])),
							(xywh[0]+dir[0]*tmpflare->pos-qdir[0]-ndir[0]),(xywh[1]-(dir[1]*tmpflare->pos-qdir[1]-ndir[1])),
							(xywh[0]+dir[0]*tmpflare->pos+qdir[0]-ndir[0]),(xywh[1]-(dir[1]*tmpflare->pos+qdir[1]-ndir[1])),
							(xywh[0]+dir[0]*tmpflare->pos+qdir[0]+ndir[0]),(xywh[1]-(dir[1]*tmpflare->pos+qdir[1]+ndir[1])),
							tmpflare->shader, tmpflare->color);
			}
...
//			trap_R_SetColor(NULL);
			...
			AdvancedDrawPicA(xywh[0],xywh[1],640.0f,480.0f,0,0,1,1,tmpflare->shader,tmpflare->color,0,3);//TURNORIGIN_MIDDLECENTER=3
			break;
		case SF_SUBLF:
			{
				vec2_t subdir;
				...
				if(curRadius==0.0f) curRadius=1.0f;
				...
				if(tmpflare->turnstyle[1]!=0.0f)
				{
					...
					if (dirlen)
					{
						...
						if(subdir[1]>0) angle=M_PI-angle;
					}
					else
						angle=0.0f;
				...
				angle+=(tmpflare->turnstyle[0]+(tmpflare->turnstyle[2]*prozDir[0]*tmpflare->pos)+(tmpflare->turnstyle[3]*prozDir[1]*tmpflare->pos))/180.0f*M_PI;
				...
				DrawLensflare(*((int *)(&tmpflare->specialVar)),
					subdir,tmpflare->color[3],distanceSquared,subxywh,qtrue);
			}
			break;
		}
...
*/
	...
	for(i=0;i<IFDA_firstempty;i++)
	{
		...
		if(lfalpha==0.0f) continue;
		DrawLensflare(IFD_Array[i].lensflare,v2,lfalpha,distanceSquared,xywh,qfalse); //normal 480 ... außer wen das bild verkleinert wird
	}
