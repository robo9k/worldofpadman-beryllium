...
//grenad_bounce ^^
	...
	case EV_SPRAYLOGO:
		...
		if ( es->eventParm != 255 )
		{
			...
			if(es->generic1==0x23)//missbrauch ;)				    vvvvvvvvvvvvv missbrauch
				Add_LogoToDrawList(es->pos.trBase,dir,logohandle,32*es->angles[0],ci);
			else
			{
				if(ci->team == TEAM_RED)
					CG_ImpactMark(cgs.media.spraymark,es->pos.trBase,dir,random()*360/*360°-winkel*/,1,0,0,1,qfalse,32*es->angles[0],qfalse);
				else if(ci->team == TEAM_BLUE)
					CG_ImpactMark(cgs.media.spraymark,es->pos.trBase,dir,random()*360/*360°-winkel*/,0,0,1,1,qfalse,32*es->angles[0],qfalse);
				else //free for sprayffa
					CG_ImpactMark(cgs.media.spraymark,es->pos.trBase,dir,random()*360/*360°-winkel*/,ci->color2[0],ci->color2[1],ci->color2[2],1,qfalse,32*es->angles[0],qfalse);
			}
...
//							CG_AddBufferedSound( cgs.media.yourTeamTookEnemyFlagSound );
	...
	case EV_POWERUP_PADPOWER:
		...
		//noch extra sound einbinden ... hmm muss ich ente noch mal nerven ;)
