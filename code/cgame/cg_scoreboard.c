...
//			CG_DrawSmallStringColor( iconx, y, string, color );
	...
	if ( score->ping == -1 ) {
		...
		{
//			CG_DrawStringExt( 640-20, y, va("%i",score->livesleft), colorWhite,qtrue,qtrue,8,16,8); 
//			if(CG_GetCvarInt("g_LPS_flags")&0x001)//LPSF_PPOINTLIMIT	0x00001
			if(cgs.lpsflags&0x001)//LPSF_PPOINTLIMIT	0x00001
				Com_sprintf(string, sizeof(string),
					"%3i/%-2i %3i %4i", score->score, score->livesleft, score->ping, score->time);
			else
				Com_sprintf(string, sizeof(string),
					"%5i %4i %4i", score->livesleft, score->ping, score->time);
		}
		else if(cgs.gametype==GT_SPRAY) //  || cgs.gametype==GT_BALLOON
			Com_sprintf(string, sizeof(string),
...
//				else if(rank+1==cg.numFFAplayers)
...
static int CG_TeamScoreboard( int y, team_t team, float fade, int maxLines, int lineHeight ) {
	...
	for ( i = 0 ; i < cg.numScores && count < maxLines ; i++ ) {
		...
		{
			if((maxLines-count)==2)
				CG_DrawStringExt(SB_SCORELINE_X,y+lineHeight*count,va("... %i Players skipped ...",(cg.scoreTeamCount[team]-maxLines+1)),colorBlack,qfalse,qfalse,8,lineHeight,40);
			if((maxLines-count)==1)
...
//				if(CG_GetCvarInt("g_LPS_flags")&0x001)//LPSF_PPOINTLIMIT	0x00001
				if(cgs.lpsflags&0x001)//LPSF_PPOINTLIMIT	0x00001
					s = va("%s"S_COLOR_BLACK" place with %iPoints & %iLives",
						CG_PlaceString( cg.snap->ps.persistant[PERS_RANK] + 1 ),
						cg.snap->ps.persistant[PERS_SCORE], cg.snap->ps.stats[STAT_LIVESLEFT] );
				else
					s = va("%s"S_COLOR_BLACK" place with %i Lives",
						CG_PlaceString( cg.snap->ps.persistant[PERS_RANK] + 1 ),
						cg.snap->ps.stats[STAT_LIVESLEFT] );
			}
			else
		...
		if ( cg.teamScores[0] == cg.teamScores[1] ) {
			...
			s = va(S_COLOR_RED"Red"S_COLOR_BLACK" leads "S_COLOR_RED"%i"S_COLOR_BLACK" to "S_COLOR_BLUE"%i",cg.teamScores[0], cg.teamScores[1] );
		} else {
			s = va(S_COLOR_BLUE"Blue"S_COLOR_BLACK" leads "S_COLOR_BLUE"%i"S_COLOR_BLACK" to "S_COLOR_RED"%i",cg.teamScores[1], cg.teamScores[0] );
		}
...
//		if(CG_GetCvarInt("g_LPS_flags")&0x001)//LPSF_PPOINTLIMIT	0x00001
	...
	if(cgs.gametype>=GT_TEAM) scoreLineSpace = (SB_STATUSBAR - SB_TOP -2*(topBorderSize+bottomBorderSize));
	else scoreLineSpace = (SB_STATUSBAR - SB_TOP);

	if(cg.scoreTeamCount[TEAM_SPECTATOR]>0) scoreLineSpace -= 8;
	...
	if(lineHeight>40) lineHeight=40;
	else if(lineHeight<10)
	{
		...
		teamDone[TEAM_FREE]		=(cg.scoreTeamCount[TEAM_FREE]<=0);
		teamDone[TEAM_RED]		=(cg.scoreTeamCount[TEAM_RED]<=0);
		teamDone[TEAM_BLUE]		=(cg.scoreTeamCount[TEAM_BLUE]<=0);
		teamDone[TEAM_SPECTATOR]=(cg.scoreTeamCount[TEAM_SPECTATOR]<=0);

		numTeams = !teamDone[TEAM_FREE] + !teamDone[TEAM_RED] + !teamDone[TEAM_BLUE] + !teamDone[TEAM_SPECTATOR];

		while(numTeams)
		{
			qboolean anyTeamDone;
...
//		int		countTeams[TEAM_NUM_TEAMS];

//		memset(countTeams, 0,sizeof(countTeams));
//		for ( i = 0 ; i < cg.numScores && (countTeams[TEAM_RED] < maxClients && countTeams[TEAM_BLUE] < maxClients); i++ ) {
//			countTeams[cgs.clientinfo[cg.scores[i].client].team]++;
//		}
