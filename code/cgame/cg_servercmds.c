...
void CG_SetConfigValues( void ) {
	...
	Q_strncpyz( cgs.balloonState, CG_ConfigString( CS_BALLOONS ), 8 );
...
void CG_UpdateBalloonStates( const char *str ) {
	...
	if ( str[0] )
	{
		...
		if ( team == '1' || team == '2' ) 
		{
			...
			// copy the new string
			Q_strncpyz( cgs.balloonState, str, 8 );
			return;
		}

		// announce normal stuff
		for ( i = 0; i < 8 && str[i] && cgs.balloonState[i]; i++ )
	...
	if ( !strcmp( cmd, "chat" ) ) {
		if ( !cg_teamChatsOnly.integer ) {
...

			{
				...
				//"NICK^7: ^2MSG" oder "[NICK^7]: ^6" ... gibt noch andere varianten...
				if(text[0]=='[')
				{
					Q_strncpyz(nick,text+1,64);
					tmpchrptr=strstr(nick,"^7]: ^");
				}
				else
				{
					Q_strncpyz(nick,text,64);
					tmpchrptr=strstr(nick,"^7: ^");
				}
				...
				if(tmpchrptr)
				{
					*tmpchrptr='\0';
					...
					{
						if(!Q_stricmp(cgs.clientinfo[i].name,nick))
						{
							cg.chaticons[cg.lastchatmsg==5?0:cg.lastchatmsg+1]=cgs.clientinfo[i].modelIcon;
							cg.chatmsgtime[cg.lastchatmsg==5?0:cg.lastchatmsg+1]=cg.time;
							break;
						}
					}
				}
...
*/
	...
	//weapon-change for sprayroom-triggers ... there were some bugs with the direct change (a switch back)
	if(!strcmp(cmd,"srwc"))//srwc->sprayroomweaponchange ;)
	{
		CG_WeaponSRWC(atoi(CG_Argv(1)));
		return;
	}

	if(!strcmp(cmd,"cdi"))//cdi->client do it ;)
	{
		switch(atoi(CG_Argv(1)))
		{
		case 0://spray room good bye ...
			trap_S_StartLocalSound(cgs.media.Announcer_SRfrag[(int)(random()*1.9999f)], CHAN_ANNOUNCER);
			break;
		case 1://sprayed on the wrong wall
//			trap_S_StartLocalSound(cgs.media.wrongLogo[(int)(random()*3.9999f)], CHAN_ANNOUNCER);
			trap_S_StartLocalSound(cgs.media.wrongLogo[atoi(CG_Argv(2))], CHAN_ANNOUNCER);
			break;
		case 2:
			trap_S_StartLocalSound(cgs.media.YouLooseSound, CHAN_ANNOUNCER);
			trap_S_StartLocalSound(cgs.media.YouLooseSound, CHAN_ANNOUNCER);//vielleicht wird es so lauter ...
			break;
		case 3://tell me ya logo ;)
			{
				char buffer[64];
				...
				if(buffer[0]!='\0')
					...
					trap_SendClientCommand(va("selectlogo 01_wop\n", buffer));
				cg.logoselected=qtrue;
			}
			break;
		case 4:
			Com_Printf("GhostPad spawned\n");
			trap_S_StartLocalSound(cgs.media.ghostPadSpawn, CHAN_ANNOUNCER);
			break;
		}
		return;
	}
