...
================
*/
void CG_Draw3DModel( float x, float y, float w, float h, qhandle_t model, qhandle_t skin, vec3_t origin, vec3_t angles, float scale)
{
	refdef_t		refdef;
	refEntity_t		ent;
...
void CG_DrawHead( float x, float y, float w, float h, int clientNum, vec3_t headAngles ) {
	...
	CG_Draw3DModel( x, y, w, h, ci->headModel, ci->headSkin, origin, headAngles, ci->headScale );
...
//old:		angles[YAW] = 60 * sin( cg.time / 2000.0 );;
		...
		CG_Draw3DModel( x, y, w, h, handle, 0, origin, angles, 1.15f );//old: 1.0f
...
//		{ 0.2, 1.0, 0.2, 1.0 } , { 1.0, 0.2, 0.2, 1.0 }, {0.5, 0.5, 0.5, 1} };
	...
	if ( cent->currentState.weapon && cg_weapons[ cent->currentState.weapon ].ammoModel ) {
		...
		if(ammo)
			CG_Draw3DModel( CHAR_WIDTH*3 + TEXT_ICON_SPACE, 432, ICON_SIZE, ICON_SIZE,
						   ammo->models[0], 0, origin, angles, 1.0f );
		else
			CG_Draw3DModel( CHAR_WIDTH*3 + TEXT_ICON_SPACE, 432, ICON_SIZE, ICON_SIZE,
					   cg_weapons[ cent->currentState.weapon ].ammoModel, 0, origin, angles, 1.0f );
	}
	...
	if ( ps->stats[ STAT_ARMOR ] ) {
		...
		CG_Draw3DModel( 370 + CHAR_WIDTH*3 + TEXT_ICON_SPACE, 432, ICON_SIZE, ICON_SIZE,
					   cgs.media.armorModel, 0, origin, angles, 1.0f );
	}
...
static int	 xpos=0;
	...
	if(cgs.gametype==GT_LPS && cgs.lpsflags&0x01)//LPSF_PPOINTLIMIT
		numLines++;
...
//	trap_Cvar_VariableStringBuffer("pointlimit",tmpstr,MAX_CVAR_VALUE_STRING);
	...
	{
		y+=SMALLCHAR_HEIGHT;
		CG_DrawStringExt(640-xpos+2,y,va("pointlimit: ^2%i", cgs.fraglimit),colorWhite,qfalse,qfalse,8,16,18);
	}
	if(cgs.gametype==GT_LPS)
	{
		y+=SMALLCHAR_HEIGHT;
		CG_DrawStringExt(640-xpos+2,y,va("LPS_startlives: ^2%i", cgs.lpsStartLives),colorWhite,qfalse,qfalse,8,16,18);
	}
...
*/
	...
	while(1){
		...
		// draw value
		strXpos = SMALLCHAR_WIDTH * CG_DrawStrlen( value );
		if( !strXpos ) break;
		strXpos = 635 - strXpos;
		CG_DrawSmallString( strXpos, y, value, 1.0F);
		
		// draw ": "
		strXpos -= SMALLCHAR_WIDTH * 2;
		CG_DrawSmallString( strXpos, y, ": ", 1.0F);

		// draw key
		strXpos -= SMALLCHAR_WIDTH * CG_DrawStrlen( key );
		CG_DrawSmallString( strXpos, y, key, 1.0F);
...
static void CG_DrawReward( void ) { 
	...
	if ( !color ) {
		if (cg.rewardStack > 0) {
			...
			cg.rewardStack--;
			color = CG_FadeColor( cg.rewardTime, REWARD_TIME );
			if(cg.rewardSound[0]!=cgs.media.impressiveSound/* && cg.rewardSound[0]!=cgs.media.excellentSound*/)//etwas umstaendlich (aber so braucht man wenig neuen src ;)
...
//	Com_Printf(":=%i\n",trap_Key_IsDown(':'));
	...
	if(!cg.logoselected && (cgs.gametype==GT_SPRAY || cgs.gametype==GT_SPRAYFFA)
		&& cgs.clientinfo[cg.clientNum].team!=TEAM_SPECTATOR)
	{
//		ActiveChooseLogoMenu();
//		return;

//noch ueberlegen ob man es nicht doch drinn lassen sollte (der server sollte jetzt eigentlich nach dem logo fragen wenn er es wirklich braucht)
//		char buffer[64];
//		trap_Cvar_VariableStringBuffer("syc_logo",buffer,sizeof(buffer));
//		trap_SendClientCommand(va("selectlogo \"%s\"\n", buffer));
//		cg.logoselected=qtrue;
	}
	else if(cg.logoselected==2)
	{
		ActiveChooseLogoMenu();
//		return;
	}
...
//		ammoWidth = UI_ProportionalStringWidth( tmpstr ) * 3 / 4;

//		ammoColor[3]=0.5f;
//		CG_FillRect(240-ammoWidth-2,360,ammoWidth+4,22,ammoColor);
...
//		CG_DrawHoldableItem(); // -> jetzt im lower right
		...
		if((cgs.gametype==GT_SPRAYFFA || cgs.gametype==GT_SPRAY) && cg.snap->ps.stats[STAT_INSPRAYROOMSEKS] <= 12)
		{
			static int lastsprayroomtime = 0;
			int tmpi;

			tmpi=cg.snap->ps.stats[STAT_INSPRAYROOMSEKS];
...
//				else if(tmpi==0)
//					trap_S_StartLocalSound(cgs.media.Announcer_SRfrag[(int)(random()*1.9999f)], CHAN_ANNOUNCER);
			...
			if(tmpi>0 && tmpi<=10)
			{
				vec4_t tmpcolor = {1,0,0,1};

				tmpcolor[1]=tmpcolor[2]=0.25f+(0.25f*sin(cg.time*0.02f));

				CG_DrawStringExt(320-64,	100, "you have",			colorWhite, qtrue, qfalse, 16, 32, 8);
				CG_DrawStringExt(320-8,		132, va("%i",tmpi),			tmpcolor,   qtrue, qfalse, 16, 32, 2);
				CG_DrawStringExt(320-56,	164, "seconds",				colorWhite, qtrue, qfalse, 16, 32, 7);
				CG_DrawStringExt(320-144,	196, "to leave this room",	colorWhite, qtrue, qfalse, 16, 32, 18);
			}
		}
...
//					}
					if(cgs.scores1 != cg.snap->ps.stats[STAT_LIVESLEFT])
					{
						hudscore2 = cg.snap->ps.stats[STAT_LIVESLEFT];
...
//			tmpangles[1] = (float)( cg.time & 3999 ) * 0.09f;// 360/4000=0.09...//( cg.time & 2047 ) * 360 / 2048.0f;
			...
			if(team == TEAM_BLUE)
				CG_Draw3DModel( x, y, w, h, cg_items[cgs.media.blueCartridgeEntNum].models[0], 0, tmporigin, tmpangles, 1.0f );
			else if(team == TEAM_RED)
				CG_Draw3DModel( x, y, w, h, cg_items[cgs.media.redCartridgeEntNum].models[0], 0, tmporigin, tmpangles, 1.0f );
			else
				CG_Draw3DModel( x, y, w, h, cg_items[cgs.media.neutralCartridgeEntNum].models[0], 0, tmporigin, tmpangles, 1.0f );
...
//:HERBY:ea
		if ( cgs.gametype == GT_BALLOON && team != TEAM_SPECTATOR )
		{
			...
			vec4_t blue = { 0.0f, 0.0f, 1.0f, 1.0f };
			vec4_t yellow = { 1.0f, 0.9f, 0.1f, 1.0f };
			vec4_t orange = { 1.0f, 0.4f, 0.1f, 1.0f };
			int i;

			// draw the icons
			for ( i = 0; i < 3 && cgs.balloonState[i]; i++ ) {
				trap_R_SetColor( NULL );
				switch ( cgs.balloonState[i] ) {
				case '1': trap_R_SetColor( red ); break;
				case '2': trap_R_SetColor( blue ); break;
				case 'a': if ( (cg.time / 400) & 1 ) trap_R_SetColor( red ); break;
				case 'b': if ( (cg.time / 400) & 1 ) trap_R_SetColor( blue ); break;
				}
...
//^^ br
		...
		if(cgs.gametype==GT_LPS && cg_drawLPSwallhack.integer)
		{
			int		i;
			clientInfo_t *ci;
			...
			for(i=0;i<cgs.maxclients;i++)
			{
				...
				if(ci->lastPosSaveTime!=0 && cg.time-ci->lastPosSaveTime<1500)
				{
					...
					// fading out, if the player disappeared(lastUpdate<=500ms 100% .. lastUpdate>=1500 0%)
					strColor[3] = 0.001f*(1500-(cg.time-ci->lastPosSaveTime));
					if(strColor[3]>1.0f) strColor[3] = 1.0f;
					else if(strColor[3]<0.0f) strColor[3] = 0.0f;

					strColor[3] *= cg_LPSwallhackAlpha.value;

					if( (SquaredDistance=Calculate_2DOf3D(ci->curPos,&cg.refdef,&x,&y)) )
					{
//						int len;
						float size;

						size=1/(sqrt((float)SquaredDistance)*0.002f);
						if(size>1.0f) size=1.0f;
						if(size<0.5f) size=0.5f;

						size *= cg_LPSwallhackSize.value;

						strColor[3] *= 0.3f+0.7f*(1/(1+SquaredDistance*0.002f*0.002f));

						if(strColor[3]>1.0f) strColor[3]=1.0f;

//						len = (int)((float)(Q_PrintStrlen(ci->name)*12)*size);
						trap_R_SetColor(strColor);
						CG_DrawPic(x-16.0f*size,y-16.0f*size,32.0f*size,32.0f*size,cgs.media.LPSwallhackicon);
						trap_R_SetColor(NULL);
/*						switch(cg_drawLPSwallhack.integer) {
						default:
						case 1:
							CG_DrawStringExt(x-len/2,y,ci->name,strColor,qfalse,qfalse,(int)(12.0f*size),(int)(24.0f*size),32);
							break;
						case 2:
							DrawTurnableString(x,y,ci->name,strColor,(20.0f*size),90,TURNORIGIN_MIDDLELEFT);
							break;
						}
*/
					}
...
//	CG_Printf("org={%1.3f|%1.3f|%1.3f}\n",cg.refdef.vieworg[0],cg.refdef.vieworg[1],cg.refdef.vieworg[2]);
	...
	if(cg.zoomed)
	{
		...
		if(cg.snap->ps.weapon!=WP_SPLASHER || cg.snap->ps.stats[STAT_HEALTH]<=0)
		{
			CG_ZoomDown_f();
		...
		//gelbe box ca. y=355-375,x=175-465
/*
		CG_DrawStringExt(180,355,va("%-3.0f",cg.snap->ps.viewangles[1]),colorYellow,qtrue,qfalse,8,16,32);
		...
		CG_DrawStringExt(320-12,355,va("%4.0f",tmpv3[0]),colorYellow,qtrue,qfalse,8,16,32);
		CG_DrawStringExt(430,355,va("%i %%",(int)(cg.zoomfactor*100.0f)),colorYellow,qtrue,qfalse,8,16,32);
*/
...
1.
		CG_AdjustFrom640(&x,&y,&w,&h);
		trap_R_DrawStretchPic(x,y,w,h,tmpv3[0],0,tmpv3[0]+0.5f,1,cgs.media.zoomcompass);
2.
		DrawPic2Color(x,y,w,h,tmpv3[0],0,tmpv3[0]+0.5f,1,colorWhite,colorWhite,cgs.media.zoomcompass);
		DrawPic2Color(x,y,w,h,tmpv3[0]+1.0f,0,tmpv3[0]+1.5f,1,colorWhite,colorWhite,cgs.media.zoomcompass);
*/
...
//		h=15.0f;
		...
		h=23.0f;
		CG_AdjustFrom640(&x,&y,&w,&h);
//		trap_R_DrawStretchPic(x,y,w,h,0.7866f*cg.zoomfactor,0,(0.7866f*cg.zoomfactor)+0.2134f,1,cgs.media.zoomruler);
...
DRAWNORMAL2D:
	...
	con_notifytime=CG_GetCvarInt("con_notifytime");
	if(cg_draw2D.integer && con_notifytime<0 && !(trap_Key_GetCatcher()&KEYCATCH_MESSAGE))
	{
		int i,j;
		...
		do
		{
			...
			if(cg.chatmsgtime[i]!=0 && cg.chatmsgtime[i]-cg.time>con_notifytime*1000)
			{
				if(cg.chaticons[i]) CG_DrawPic(0,j*14,14,14,cg.chaticons[i]);
				CG_DrawStringExt(15,2+j*14,cg.chattext[i],colorWhite,qfalse,qfalse,6,12,80);
				j++;
			}
...
/*
	CG_DrawStringExt(19+(int)(random()*3.9999f),99+(int)(random()*3.9999f),"8-> 123456789 ABCDEFGHIJKLMNOPQRSTUVWXYZ abcdefghijklmnopqrstuvwxyz",colorBlack,qfalse,qfalse,4,8,80);
	CG_DrawStringExt(20,100,"8-> 123456789 ABCDEFGHIJKLMNOPQRSTUVWXYZ abcdefghijklmnopqrstuvwxyz",colorWhite,qfalse,qfalse,4,8,80);

	CG_DrawStringExt(19+(int)(random()*3.9999f),119+(int)(random()*3.9999f),"10-> 123456789 ABCDEFGHIJKLMNOPQRSTUVWXYZ abcdefghijklmnopqrstuvwxyz",colorBlack,qfalse,qfalse,5,10,80);
	CG_DrawStringExt(20,120,"10-> 123456789 ABCDEFGHIJKLMNOPQRSTUVWXYZ abcdefghijklmnopqrstuvwxyz",colorWhite,qfalse,qfalse,5,10,80);

	CG_DrawStringExt(19+(int)(random()*3.9999f),139+(int)(random()*3.9999f),"12-> 123456789 ABCDEFGHIJKLMNOPQRSTUVWXYZ abcdefghijklmnopqrstuvwxyz",colorBlack,qfalse,qfalse,6,12,80);
	CG_DrawStringExt(20,140,"12-> 123456789 ABCDEFGHIJKLMNOPQRSTUVWXYZ abcdefghijklmnopqrstuvwxyz",colorWhite,qfalse,qfalse,6,12,80);
*/
}
