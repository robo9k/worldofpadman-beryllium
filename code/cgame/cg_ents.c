...
static void CG_StationHealth( vec3_t origin, int value ) {
...
// bringt hier doch garnix O_o ... moved to the right position ;P
//			if(cent->currentState.angles2[1]==0.0f)
//				trap_S_StopLoopingSound(cent->currentState.number); //ich hoffe das hilft (die loop lief manchmal wenn eigentlich keiner drauf stand)
		}
...
/*
	temporig[0]=cent->lerpOrigin[0];
	temporig[1]=cent->lerpOrigin[1];
	temporig[2]=cent->lerpOrigin[2]+64+32;
	CG_StationHealth(temporig,cent->currentState.apos.trDuration);

	temporig[2]+=32;
	CG_StationHealth(temporig,(int)cent->currentState.angles2[1]);
*/
}

static void CG_DrawGhostPad( centity_t *cent )
{
	refEntity_t		ent;
...
//	AxisClear(ent.axis);
	{
		...
		VectorCopy(cent->currentState.pos.trDelta, ent.axis[0]);
	...
	ent.reType = RT_MODEL;
	ent.hModel = cgs.media.GhostPadModel;

	ent.oldframe=cent->lastweaponframe;
	if(cent->currentState.eFlags & EF_DEAD)
	{
		ent.frame = 63+(cg.time-cent->currentState.time)/40;
		if(ent.frame>85)
		{
			vec3_t tmp;
			int time = cg.time-cent->currentState.time - 880;
			ent.frame = 85;
			ent.customShader = cgs.media.deadfadeSkin;
			if(time>=4000)	ent.shaderRGBA[3]=0;
			else
//				ent.shaderRGBA[3]=0xff-(0xff*time/4000);
				ent.shaderRGBA[3]=0xff-(0xcc*time/4000);
			// TODES-ANIMATION ...
			ent.origin[2] += (time)/20.0f;
			vectoangles(ent.axis[0],tmp);
			tmp[YAW] += (time)/10.0f;
			AnglesToAxis(tmp,ent.axis);
		}
	}
	else
	{
		if(cent->nextspeedytime==0)
			cent->nextspeedytime=cg.time+2000;
		if(cent->nextspeedytime<cg.time)
		{
			if(cent->nextspeedytime+1240>cg.time)
				ent.frame = 32+(cg.time-cent->nextspeedytime)/40;
			else
				cent->nextspeedytime=cg.time+20000;//+crandom()*bla

			if(ent.frame==32)
				trap_S_StartSound(NULL,cent->currentState.number,CHAN_VOICE,cgs.media.ghostPadSpook);
		}
		else
			ent.frame = 31&((cent->nextspeedytime-cg.time)/40);

		if(cent->currentState.eFlags & EF_FIRING)
		{
			int time = cg.time-cent->currentState.time;
			ent.origin[2] += (time)/4.0f;
			ent.shaderRGBA[3] = 0xff*(2000-time)/2000;
			ent.renderfx = RF_FORCEENTALPHA;
		}
	}

	cent->lastweaponframe=ent.frame;
	VectorCopy( ent.origin, ent.oldorigin );//brauchts das?
