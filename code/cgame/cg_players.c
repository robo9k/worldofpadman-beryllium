...
static qboolean	CG_ParseAnimationFile( const char *filename, clientInfo_t *ci ) {
	...
	for ( i = 0 ; i < MAX_ANIMATIONS ; i++ ) {
		...
		if ( !*token ) {
			...
			{
				animations[TORSO_SPRAYATTACK].firstFrame	= animations[TORSO_ATTACK].firstFrame;
				animations[TORSO_SPRAYATTACK].frameLerp		= animations[TORSO_ATTACK].frameLerp;
				animations[TORSO_SPRAYATTACK].initialLerp	= animations[TORSO_ATTACK].initialLerp;
				animations[TORSO_SPRAYATTACK].loopFrames	= animations[TORSO_ATTACK].loopFrames;
				animations[TORSO_SPRAYATTACK].numFrames		= animations[TORSO_ATTACK].numFrames;
				animations[TORSO_SPRAYATTACK].reversed		= animations[TORSO_ATTACK].reversed;
				animations[TORSO_SPRAYATTACK].flipflop		= animations[TORSO_ATTACK].flipflop;
				continue;
			}
			else if(i==TORSO_SPRAYSTAND)
			{
				animations[TORSO_SPRAYSTAND].firstFrame		= animations[TORSO_STAND].firstFrame;
				animations[TORSO_SPRAYSTAND].frameLerp		= animations[TORSO_STAND].frameLerp;
				animations[TORSO_SPRAYSTAND].initialLerp	= animations[TORSO_STAND].initialLerp;
				animations[TORSO_SPRAYSTAND].loopFrames		= animations[TORSO_STAND].loopFrames;
				animations[TORSO_SPRAYSTAND].numFrames		= animations[TORSO_STAND].numFrames;
				animations[TORSO_SPRAYSTAND].reversed		= animations[TORSO_STAND].reversed;
				animations[TORSO_SPRAYSTAND].flipflop		= animations[TORSO_STAND].flipflop;
				continue;
			}
			else if(i==LEGS_FLYING)
			{
				animations[LEGS_FLYING].firstFrame			= animations[LEGS_IDLE].firstFrame;
				animations[LEGS_FLYING].frameLerp			= animations[LEGS_IDLE].frameLerp;
				animations[LEGS_FLYING].initialLerp			= animations[LEGS_IDLE].initialLerp;
				animations[LEGS_FLYING].loopFrames			= animations[LEGS_IDLE].loopFrames;
				animations[LEGS_FLYING].numFrames			= animations[LEGS_IDLE].numFrames;
				animations[LEGS_FLYING].reversed			= animations[LEGS_IDLE].reversed;
				animations[LEGS_FLYING].flipflop			= animations[LEGS_IDLE].flipflop;
				continue;
			}
			else if(i==LEGS_HEAVYLAND)
			{
				animations[LEGS_HEAVYLAND].firstFrame		= animations[LEGS_JUMP].firstFrame;
				animations[LEGS_HEAVYLAND].frameLerp		= animations[LEGS_JUMP].frameLerp;
				animations[LEGS_HEAVYLAND].initialLerp		= animations[LEGS_JUMP].initialLerp;
				animations[LEGS_HEAVYLAND].loopFrames		= animations[LEGS_JUMP].loopFrames;
				animations[LEGS_HEAVYLAND].numFrames		= animations[LEGS_JUMP].numFrames;
				animations[LEGS_HEAVYLAND].reversed			= !animations[LEGS_JUMP].reversed;
				animations[LEGS_HEAVYLAND].flipflop			= animations[LEGS_JUMP].flipflop;
				continue;
			}
			break;
		}
...
static qboolean	CG_FindClientModelFile( char *filename, int length, clientInfo_t *ci, const char *teamName, const char *modelName, const char *skinName, const char *base, const char *ext ) {
	...
	if ( cgs.gametype >= GT_TEAM ) {
		...
		if(strstr(skinName,"_blue") || strstr(skinName,"_red")) {
			char* ptr;
			Q_strncpyz(tmpSkin,skinName,MAX_QPATH);
	...
	while(1) {
		for ( i = 0; i < 2; i++ ) {
			if ( i == 0 && teamName && *teamName ) {
				...
				Com_sprintf( filename, length, "models/wop_players/%s%s/%s%s_%s_%s.%s", charactersFolder, modelName, teamName, base, skinName, team, ext );
			}
			else {
				//								"models/players/characters/james/lower_lily_red.skin"
				Com_sprintf( filename, length, "models/wop_players/%s%s/%s_%s_%s.%s", charactersFolder, modelName, base, skinName, team, ext );
			}
			...
			if ( cgs.gametype >= GT_TEAM ) {
				if ( i == 0 && teamName && *teamName ) {
					...
					Com_sprintf( filename, length, "models/wop_players/%s%s/%s%s_%s.%s", charactersFolder, modelName, teamName, base, team, ext );
				}
				else {
					//								"models/players/characters/james/lower_red.skin"
					Com_sprintf( filename, length, "models/wop_players/%s%s/%s_%s.%s", charactersFolder, modelName, base, team, ext );
				}
			}
			else {
				if ( i == 0 && teamName && *teamName ) {
					//								"models/players/characters/james/stroggs/lower_lily.skin"
					Com_sprintf( filename, length, "models/wop_players/%s%s/%s%s_%s.%s", charactersFolder, modelName, teamName, base, skinName, ext );
				}
				else {
					//								"models/players/characters/james/lower_lily.skin"
					Com_sprintf( filename, length, "models/wop_players/%s%s/%s_%s.%s", charactersFolder, modelName, base, skinName, ext );
				}
			}
			if ( CG_FileExists( filename ) ) {
				return qtrue;
...
//	int i;
	...
	while(1) {
		for ( i = 0; i < 2; i++ ) {
			if ( i == 0 && teamName && *teamName ) {
				Com_sprintf( filename, length, "models/wop_players/%s%s/%s/%s%s_%s.%s", headsFolder, headModelName, headSkinName, teamName, base, team, ext );
			}
			else {
				Com_sprintf( filename, length, "models/wop_players/%s%s/%s/%s_%s.%s", headsFolder, headModelName, headSkinName, base, team, ext );
			}
			if ( CG_FileExists( filename ) ) {
				return qtrue;
			}
			if ( cgs.gametype >= GT_TEAM ) {
				if ( i == 0 &&  teamName && *teamName ) {
					Com_sprintf( filename, length, "models/wop_players/%s%s/%s%s_%s.%s", headsFolder, headModelName, teamName, base, team, ext );
				}
				else {
					Com_sprintf( filename, length, "models/wop_players/%s%s/%s_%s.%s", headsFolder, headModelName, base, team, ext );
				}
			}
			else {
				if ( i == 0 && teamName && *teamName ) {
					Com_sprintf( filename, length, "models/wop_players/%s%s/%s%s_%s.%s", headsFolder, headModelName, teamName, base, headSkinName, ext );
				}
				else {
					Com_sprintf( filename, length, "models/wop_players/%s%s/%s_%s.%s", headsFolder, headModelName, base, headSkinName, ext );
				}
			}
			if ( CG_FileExists( filename ) ) {
				return qtrue;
...
static qboolean CG_RegisterClientModelname( clientInfo_t *ci, const char *modelName, const char *skinName, const char *headModelName, const char *headSkinName, const char *teamName ) {
	...
	if ( !ci->legsModel ) {
		Com_sprintf( filename, sizeof( filename ), "models/wop_players/characters/%s/lower.md3", modelName );
		ci->legsModel = trap_R_RegisterModel( filename );
		if ( !ci->legsModel ) {
	...
	if ( !ci->torsoModel ) {
		Com_sprintf( filename, sizeof( filename ), "models/wop_players/characters/%s/upper.md3", modelName );
		ci->torsoModel = trap_R_RegisterModel( filename );
		if ( !ci->torsoModel ) {
			Com_Printf( "Failed to load model file %s\n", filename );
			return qfalse;
	...
	if( headName[0] == '*' ) {
		Com_sprintf( filename, sizeof( filename ), "models/wop_players/heads/%s/%s.md3", &headModelName[1], &headModelName[1] );
	}
	else {
		Com_sprintf( filename, sizeof( filename ), "models/wop_players/%s/head.md3", headName );
	}
	ci->headModel = trap_R_RegisterModel( filename );
	// if the head model could not be found and we didn't load from the heads folder try to load from there
	if ( !ci->headModel && headName[0] != '*' ) {
		Com_sprintf( filename, sizeof( filename ), "models/wop_players/heads/%s/%s.md3", headModelName, headModelName );
		ci->headModel = trap_R_RegisterModel( filename );
	}
	if ( !ci->headModel ) {
		Com_Printf( "Failed to load model file %s\n", filename );
	...
	if ( !CG_ParseAnimationFile( filename, ci ) ) {
		Com_sprintf( filename, sizeof( filename ), "models/wop_players/characters/%s/animation.cfg", modelName );
		if ( !CG_ParseAnimationFile( filename, ci ) ) {
...
static void CG_LoadClientInfo( clientInfo_t *ci ) {
	...
	for ( i = 0 ; i < MAX_CUSTOM_SOUNDS ; i++ ) {
		...
		if (modelloaded) {
			ci->sounds[i] = trap_S_RegisterSound( va("sound/wop_player/%s/%s", dir, s + 1), qfalse );
		}
		if ( !ci->sounds[i] ) {
			ci->sounds[i] = trap_S_RegisterSound( va("sound/wop_player/%s/%s", fallback, s + 1), qfalse );
		}
	}
...
void CG_NewClientInfo( int clientNum ) {
	...
	if ( !configstring[0] ) {
		//noch zu testen ob das wo anderes "bugs" erzeugt (will nur die infos für den TeleOut noch nutzen)
		ci->infoValid = qfalse;
	...
	v = Info_ValueForKey( configstring, "c2" );
	{
		...
		else if(colorid>5) colorid=5;

		newInfo.color2[0] = spraycolors[colorid][0];
		newInfo.color2[1] = spraycolors[colorid][1];
		newInfo.color2[2] = spraycolors[colorid][2];
	}
	...
	Com_sprintf(newInfo.spraylogo,32,"%s",v);
...
{
	int				i;
	vec3_t			tmpv3, zwischenPos;
	int				SquaredLength;
static int			lastLoopSoundTime=0;

	if ( cent->nextspeedytime > cg.time ) {
		return;
	}

	tmpv3[0] = cent->speedyorigin[0] - cent->lerpOrigin[0];
	tmpv3[1] = cent->speedyorigin[1] - cent->lerpOrigin[1];
	tmpv3[2] = cent->speedyorigin[2] - cent->lerpOrigin[2];
	SquaredLength=VectorLengthSquared(tmpv3);

	if ( (SquaredLength>16384.0f) ) {//abstand größer als 128 units
		cent->speedyorigin[0] = cent->lerpOrigin[0];
		cent->speedyorigin[1] = cent->lerpOrigin[1];
		cent->speedyorigin[2] = cent->lerpOrigin[2];
		cent->nextspeedytime += 100;
		return;
	}
	
	if(SquaredLength<1024.0f)//kleinere als 32
	{
		cent->nextspeedytime += 100;
		return;
	}
...

	{
		for(i=0;i<5;i++)
		{
			zwischenPos[0]=cent->lerpOrigin[0]+tmpv3[0]*i/5;
			zwischenPos[1]=cent->lerpOrigin[1]+tmpv3[1]*i/5;
			zwischenPos[2]=(cent->lerpOrigin[2]+tmpv3[2]*i/5)-20;

			LaunchSpeedyPuffTrail(zwischenPos);
		}
	}
...
using the harvester-code from above
===============
*/
...
//		CG_SmokePuff( cent->lerpOrigin, vec3_origin, 20, 0.5f, 0.5f, 1, 0.4f, 2000, cg.time, 0, 0, cgs.media.smokePuffShader );
//		LaunchFloaterPuff(cent->lerpOrigin);
		CG_SmokePuff( cent->lerpOrigin, vec3_origin, 20, 0.2f, 0.0f, 0.4f, 0.5f, 2000, cg.time, 0, 0, cgs.media.smokePuffShader );
	}
...
void CG_AddRefEntityWithPowerups( refEntity_t *ent, entityState_t *state, int team )
{
	...
	else if ( state->powerups & ( 1 << PW_PADPOWER ) )
	{
		...
		if (team == TEAM_RED)
		{ ent->shaderRGBA[0]=0xff;ent->shaderRGBA[1]=ent->shaderRGBA[2]=0; }
		else if(team == TEAM_BLUE)
		{ ent->shaderRGBA[0]=ent->shaderRGBA[1]=0;ent->shaderRGBA[2]=0xff; }
		else
		{ ent->shaderRGBA[0]=ent->shaderRGBA[1]=ent->shaderRGBA[2]=0xff; }
	}
	...
	if(state->eFlags & EF_DEAD && state->time)
	{
		...
		if(!state->time)					ent->shaderRGBA[3]=0xff;
		else if(cg.time-state->time>=4000)	ent->shaderRGBA[3]=0;
		else
			ent->shaderRGBA[3]=0xff-(0xff*(cg.time-state->time)/4000.0f);
	}
	trap_R_AddRefEntityToScene( ent );

	if(state->eType == ET_PLAYER)
	{
		int ltiT = cgs.clientinfo[state->clientNum].lastTeleInTime;

		if(!(state->eFlags & EF_DEAD) && ltiT && cg.time-ltiT<1500)
		{
			float f = (cg.time-ltiT)/1500.0f;
			vec3_t	axisBU[3];

			if ( team == TEAM_RED ) ent->customShader = cgs.media.teleportEffectRedShader;
			else if ( team == TEAM_BLUE ) ent->customShader = cgs.media.teleportEffectBlueShader;
			else ent->customShader = cgs.media.teleportEffectGreenShader;
			
			ent->shaderRGBA[0]=ent->shaderRGBA[1]=ent->shaderRGBA[2]=0xFF*(1.0f-f);

			AxisCopy(ent->axis,axisBU);
			AxisScale( ent->axis, (1.01f + f*0.3f), ent->axis );			
			trap_R_AddRefEntityToScene( ent );
			AxisCopy(axisBU,ent->axis);
		}
	}
}
...
void CG_Player( centity_t *cent ) {
	...
	qboolean		shadow;
	float			shadowPlane;
	float			lerpOri2=cent->lerpOrigin[2];
	float			angleBU =cent->lerpAngles[YAW];
	...
	{
		if(cg.time-cent->currentState.time>=4000)
			return; //noch mal überdenken ...
		else
		{
			cent->lerpOrigin[2] += (cg.time-cent->currentState.time)/20.0f;
			cent->lerpAngles[YAW] += (cg.time-cent->currentState.time)/10.0f;
		}
	}
	...
	cent->lerpOrigin[2]  = lerpOri2;
	cent->lerpAngles[YAW]= angleBU;
	...
	if ( cent->currentState.eFlags & (EF_DEAD | EF_HURT) && !(cent->currentState.powerups & (1<<PW_VISIONLESS)))
	{
		...
		if(cent->currentState.eFlags & EF_DEAD && cent->currentState.time)
		{
			if(cg.time-cent->currentState.time<=1000)
				alpha = 1.0f - (cg.time-cent->currentState.time)/1000.0f;
			else
				alpha = 0.0f;
		}
	...
	// Save player position (for LPS "wallhack"-names)
	if(cgs.gametype==GT_LPS &&
		!(cgs.lpsflags&0x004) && //LPSF_NONAMES
	...
	else if(ci->lastPosSaveTime!=cg.time)//so we don't lose the overheadname, if the copiedbody comes after the new player
...
//		vec3_t tmpv3;
//
//		tmpv3[0] = cent->lerpOrigin[0];
//		tmpv3[1] = cent->lerpOrigin[1];
//		tmpv3[2] = cent->lerpOrigin[2]+64;
//		LaunchSpiralParticle(tmpv3);
//	}
