...
void BE_ClientUserinfoChanged( int clientNum ) {
	...
	Q_strncpyz( model, Info_ValueForKey( userinfo, modelInfo ),
	            sizeof( model ) );
	Q_strncpyz( headmodel, Info_ValueForKey( userinfo, headmodelInfo ),
	            sizeof( headmodel ) );

	if ( !validPlayermodel( model, headmodel ) ) {
		/* TODO: Add a default model for teamgames? */
		Info_SetValueForKey( userinfo, modelInfo,
		                     DEFAULT_PLAYERMODEL_S );
		Info_SetValueForKey( userinfo, headmodelInfo,
		                     DEFAULT_PLAYERMODEL_S );
		changed = qtrue;
	}

	/* Used twice below. */
	ClientCleanName( Info_ValueForKey( userinfo, "name" ), name,
	                 sizeof( name ) );
	...
	if ( strcmp( name, oldname ) != 0 ) {
		...
		if ( ( be_maxNameChanges.integer != -1 ) &&
			...
			SendCmd( clientNum, CCMD_PRT,
			         va( S_COLOR_NEGATIVE
			             "You reached the maximum number of %i renames.\n",
	...
	if ( !( ent->r.svFlags & SVF_BOT )
	     && ( be_settings.integer & BE_UNIQUENAMES ) ) {
		Q_strncpyz( ent->client->pers.netname, name,
		            sizeof( ent->client->pers.netname ) );
		if ( ClientnumFromString( name ) == CID_MULTIPLE ) {
			char newname[MAX_NETNAME];
			for ( i = 0; i < level.maxclients; i++ ) {
				Com_sprintf( newname, sizeof( newname ),
				             RENAMED_PLAYERNAME_S" %i", ( i + 1 ) );
				if ( ClientnumFromString( newname ) == CID_NONE ) {
					Info_SetValueForKey( userinfo, "name", newname );
					changed = qtrue;
					Q_strncpyz( ent->client->pers.netname, newname,
					            sizeof( ent->client->pers.netname ) );
					/*
					...
					if ( ( CON_CONNECTED == ent->client->pers.connected )
						...
						SendCmd( CID_ALL, CCMD_PRT,
						         va( S_COLOR_ITALIC"%s"S_COLOR_ITALIC
								     " was automatically renamed to %s"
									 S_COLOR_ITALIC".\n",
						             oldname, newname ) );
					}
					break;
			 ...
			 */
			Q_strncpyz( ent->client->pers.netname, oldname,
			            sizeof( ent->client->pers.netname ) );
		}
	}
...
static void DisableProtection( gentity_t *ent ) {
	...
	if ( ent->client->pers.isProtected ) {
		...
		remaining = ( ( be_respawnProtect.integer * 1000 )
...
void BE_ClientTimerActions( gentity_t* ent ) {
	...
	if ( DistanceSquared( ent->client->pers.campPosition, position )
	     < Square( be_campDistance.value ) ) {
	...
	if ( ( remaining <= 10 ) && ( remaining > 5 ) ) {
		SendCmd( ( ent - g_entities ),
		         CCMD_CP, S_COLOR_ITALIC"Camping will get you killed.." );
	}
	else if ( ( remaining <= 5 ) && ( remaining >= 1 ) ) {
		SendCmd( ( ent - g_entities ),
		         CCMD_CP, S_COLOR_BOLD"Get moving!" );
	}
	 ...
	 *        is set.
	 */
	counter = ( ( ent->client->pers.realPing < 999 )
	            ? ent->client->pers.realPing : 999 );
	...
	if ( ent->client->pers.isProtected ) {
		if ( ( level.time - ent->client->respawnTime )
		     > ( be_respawnProtect.integer * 1000 ) ) {
...
void BE_ClientBegan( int clientNum ) {
	...
	if ( smoothClients ) {
		...
		G_Say( NULL, ent, SAY_TELL,
		       S_COLOR_NEGATIVE"Please disable "
			   S_COLOR_BOLD"cg_smoothClients" S_COLOR_NEGATIVE
	...
	if ( ( GT_LPS == g_gametype.integer ) &&
		...
		if ( killer_ent->client && ( killer_ent != self ) ) {
			...
			if ( numShards >= neededShards ) {
				...
				SendCmd( CID_ALL, CCMD_PRT,
				         va( "%s"S_COLOR_DEFAULT" gained an extra life!\n",
						     killer->pers.netname ) );
			}
			else {
...
void BE_InitBeryllium( void ) {
	 ...
	 */
	Q_strncpyz( level.mapname, Info_ValueForKey( serverinfo, "mapname" ),
	            sizeof( level.mapname ) );
...
qboolean BE_CanUseTeleporter( const gentity_t *ent, gentity_t *other ) {
	if ( be_noSecrets.integer ) {
		...
		if ( !allowed ) {
			...
			SelectSpawnPoint( other->r.currentOrigin,
			                  origin, angles, qfalse );
 ...
 */
qboolean BE_HideChat( gentity_t *ent, gentity_t *target, int mode,
                      int color, const char *name, const char *message ) {
	int len;
...
void BE_PreSpawnEntities( void ) {
	...
	trap_GetServerinfo( serverinfo, sizeof( serverinfo ) );
	Q_strncpyz( mapname, Info_ValueForKey( serverinfo, "mapname" ),
	            sizeof( mapname ) );
...
void BE_ClientSpawned( gentity_t *ent ) {
	...
	if ( !IsSpectator( ent->client ) ) {
		if ( be_respawnProtect.integer > 0 ) {
			...
			ent->client->ps.powerups[PW_VISIONLESS] =
				( level.time + be_respawnProtect.integer * 1000 );
		}
	}
}
