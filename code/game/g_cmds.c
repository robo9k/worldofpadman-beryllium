...
void Cmd_Give_f (gentity_t *ent)
{
	...
	if (give_all || Q_stricmp(name, "weapons") == 0)
	{
		ent->client->ps.stats[STAT_WEAPONS] |= (1 << WP_NUM_WEAPONS) - 1 - 
			( 1 << WP_GRAPPLING_HOOK ) - ( 1 << WP_NONE ) - ( 1 << WP_KILLERDUCKS ) - ( 1 << WP_SPRAYPISTOL );
		if (!give_all)
			return;
	}
...
void SetTeam( gentity_t *ent, char *s ) {
	...
	if(g_gametype.integer==GT_LPS && client->ps.stats[STAT_LIVESLEFT]<=0)
		lpsNoLives = qtrue;
...
void Cmd_Follow_f( gentity_t *ent ) {
	...
	if ( level.clients[ i ].sess.sessionTeam == TEAM_SPECTATOR || (g_gametype.integer==GT_LPS && level.clients[ i ].sess.livesleft<=0)) {
		return;
	}
...
void Cmd_FollowCycle_f( gentity_t *ent, int dir ) {
	...
	do {
		...
		// can't follow another spectator
		if ( level.clients[ clientnum ].sess.sessionTeam == TEAM_SPECTATOR || (g_gametype.integer==GT_LPS && level.clients[ clientnum ].sess.livesleft<=0)) {
			continue;
		}
...
void Cmd_CallVote_f( gentity_t *ent ) {
	...
	} else if ( !Q_stricmp( arg1, "fastGameSpeed" ) ) {
	} else if ( !Q_stricmp( arg1, "normalGameSpeed" ) ) {
	} else {
		trap_SendServerCommand( ent-g_entities, "print \"Invalid vote string.\n\"" );
		trap_SendServerCommand( ent-g_entities, "print \"Vote commands are: map_restart, nextmap, map <mapname>, g_gametype <n>, kick <player>, clientkick <clientnum>, g_doWarmup, timelimit <time>, pointlimit <points>.\n\"" );
		return;
	}
	...
	if ( !Q_stricmp( arg1, "g_gametype" ) ) {
		...
		Com_sprintf( level.voteString, sizeof( level.voteString ), "set g_speed 320");
		Com_sprintf( level.voteDisplayString, sizeof( level.voteDisplayString ), "fast gamespeed(320 like q3a or oldWoP)" );
	} else if ( !Q_stricmp( arg1, "normalGameSpeed" ) ) {
		Com_sprintf( level.voteString, sizeof( level.voteString ), "set g_speed "DEFAULT_GSPEED_STR);
		Com_sprintf( level.voteDisplayString, sizeof( level.voteDisplayString ), "normal gamespeed("DEFAULT_GSPEED_STR")" );
	} else {
		Com_sprintf( level.voteString, sizeof( level.voteString ), "%s \"%s\"", arg1, arg2 );
		Com_sprintf( level.voteDisplayString, sizeof( level.voteDisplayString ), "%s", level.voteString );
	}
...
*/
	...
	trap_SendServerCommand(ent-g_entities, va("print \"%2s %3s %-32s %-32s\n\"","ID","Bot","name","syc-logo" ));
	for(i=0;i<g_maxclients.integer;i++)
	{
		if(level.clients[i].pers.connected == CON_CONNECTED)
			trap_SendServerCommand(ent-g_entities, va("print \"%2i %-3i %-32s %-32s\n\"",i,(g_entities[i].r.svFlags&SVF_BOT)/SVF_BOT,level.clients[i].pers.netname,level.clients[i].sess.selectedlogo ));
	}
}
...
{
	if(!ent->client) return;//this should never happen, but if it does we wont need an error

	trap_Argv(1,ent->client->sess.selectedlogo,32);
